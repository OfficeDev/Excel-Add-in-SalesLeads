...
/***/ function(module, exports) {
	...
	(function () {
	    ...
	    var SalesLeadApp = window.SalesLeadApp || {};
	    ...
	    SalesLeadApp.getData = function (event) {
	        ...
	        var salesResponse = $.ajax({
	            url: 'MockDynamicsData/SalesLeadsData.json',
	            dataType: 'json',
	            cache: false
	        });

	        var customerResponse = $.ajax({
	            url: 'MockDynamicsData/Customers.json',
	            dataType: 'json',
	            cache: false
	        });

	        $.when(salesResponse, customerResponse).then(function (resolvedSalesLeadData, resolvedCustomerData) {

	            // The sales lead and customer data is converted from JSON
	            // to arrays that can populate Ranges.
	            var salesLeadData = resolvedSalesLeadData[0];
	            var customerData = resolvedCustomerData[0];

	            // Return them as a single object so they can be passed to
	            // the callback of the next "then".
	            return { salesLeadData: salesLeadData, customerData: customerData };
	        }).then(function (_ref) {
	            var salesLeadData = _ref.salesLeadData,
	        ...
	        function createCustomersSheetAndTable(sheets, data) {
	            ...
	            var customersTable = customerSheet.tables.add('A1:E1', true);
	            customersTable.name = 'CustomersTable';
	            customersTable.getHeaderRowRange().values = [['Name', 'Phone', 'City', 'Primary Contact', 'Email']];

	            var tableBodyData = data.map(function (customer) {
	                return [customer.Name, customer.Phone, customer.City, customer.PrimaryContact, customer.Email];
	            });
	        ...
	        function createOpportunitySheetAndTable(sheets, data) {
	            var opportunitySheet = sheets.add();
	            ...
	            var salesLeadTable = opportunitySheet.tables.add('A1:F1', true);
	            salesLeadTable.name = 'SalesLeadTable';
	            salesLeadTable.getHeaderRowRange().values = [['Owner', 'Account', 'Topic', 'Probability', 'Est. Close Date', 'Est. Revenue']];
	            ...
	            var tableBodyData = data.map(function (lead) {
	                return [lead.Owner, lead.Account, lead.Topic, lead.Probability, lead.EstCloseDate, lead.EstRevenue];
	            });
	    ...
	    SalesLeadApp.analyse = function (event) {
	        return Excel.run(function (context) {
	            ...
	            var selectedSalesPersonRange = context.workbook.getSelectedRange().load('values');
	            var saleLeadRows = context.workbook.tables.getItem('SalesLeadTable').rows.load('count, values');

	            return context.sync().then(function () {
	                SalesLeadApp.selectedSalesPerson = selectedSalesPersonRange.values[0][0];

	                for (var i = 0; i < saleLeadRows.count; i++) {
	                    var row = saleLeadRows.items[i];
	                    if (row.values[0][0] === SalesLeadApp.selectedSalesPerson &&
	        ...
	        function getFilteredData(sheets) {
	            ...
	            var salesLeadTable = sheets.getItem('Opportunities').tables.getItem('SalesLeadTable');
	            var filteredClosedSales = filterSalesLeads(sheets, salesLeadTable);
	            ...
	            function filterSalesLeads(sheets, tableToFilter) {
	                var ownerFilter = tableToFilter.columns.getItem('Owner').filter;
	                ownerFilter.applyValuesFilter([SalesLeadApp.selectedSalesPerson]);
	                var dateFilter = tableToFilter.columns.getItem('Est. Close Date').filter;
	        ...
	        function createTempSheet(sheets) {
	            ...
	            var range = tempSheet.getRange('A1:F' + SalesLeadApp.closedSales.length);
	            range.values = SalesLeadApp.closedSales;
	            var dateRange = tempSheet.getRange('E1:E' + SalesLeadApp.closedSales.length);
	        ...
	        function createAnalysisSheet(sheets, data) {
	            var analysisSheet = sheets.add();
	            analysisSheet.name = SalesLeadApp.selectedSalesPerson;
	            analysisSheet.tabColor = 'green';

	            var salesPersonLeadsTable = analysisSheet.tables.add('A3:D3', true);

	            // Table names must be unique across the entire workbook, and may not
	            // contain spaces.
	            var tableName = SalesLeadApp.selectedSalesPerson + 'LeadsTable';
	            tableName = tableName.replace(" ", "");
	            salesPersonLeadsTable.name = tableName;
	            salesPersonLeadsTable.getHeaderRowRange().values = [['Account', 'Probability', 'Est. Revenue', 'Expected Value']];

	            var newData = data.map(function (item, index) {
	                return ['', // The values of this column will be hyperlinks created below.
	                item.Probability, item.EstRevenue,
	            ...
	            data.forEach(function (opportunity, index) {
	                SalesLeadApp.customerRowAddresses.forEach(function (customerRowAddress) {
	                    if (opportunity.Account === customerRowAddress.Name) {
	                        var range = analysisSheet.getRange('A' + ('' + (index + 4)));
	                        var hyperlink = {
	            ...
	            // potential values.
	            var rangeToFormat = analysisSheet.getRange('D3:D' + (newData.length + 3));
	            var conditionalFormat = rangeToFormat.conditionalFormats.add(Excel.ConditionalFormatType.colorScale);
	            var criteria = {
	                minimum: {
	                    formula: null,
	            ...
	            var tableTitleRange = analysisSheet.getRange('B1');
	            ...
	            var hiddenSheet = sheets.getItem(SalesLeadApp.selectedSalesPerson + 'HiddenTemp');

	            // closedSales holds the closed sales from last year for the salesperson.
	            var dataRange = hiddenSheet.getRange('E1:F' + SalesLeadApp.closedSales.length);
	            var chart = analysisSheet.charts.add('ColumnClustered', dataRange, 'auto');
	            ...
	            var trendline = chart.series.getItemAt(0).trendlines.getItem(0);
	        ...
	        function getFileContentsHelper() {
	            file.getSliceAsync(fileSlices.length, function (result) {
	                ...
	                if (fileSlices.length == expectedSliceCount) {
	                    ...
	                    var array = [];
	                    fileSlices.forEach(function (slice) {
